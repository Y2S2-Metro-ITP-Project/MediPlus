...
import {  HiSearch, HiFilter, HiTrash, HiEye, HiDocumentReport } from "react-icons/hi";
...
export default function DashSlot() {
  ...
  const handleAddSlot = async (e) => {
    ...
    try {
      ...
      if (resSlot.ok) {
        ...
        toast.error(dataSlot.message || "Failed to create slot and schedule appointments.");
      }
    } catch (error) {
      console.log(error);
      toast.error(error.message || "An error occurred while creating slot and scheduling appointments.");
    }
  };
  ...
  const handleCancel = async () => {
    try {
      ...
      toast.error("An error occurred while cancelling the slot. Please try again later.");
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.error("An error occurred while deleting the slot. Please try again later.");
    }
  };
  ...
  const generateTotalReport = async () => {
    try {
      ...
      toast.error("Failed to generate total slots report. Please try again later.");
    }
  };

  const filteredSlots = slots.filter((slot) => {
    const matchesSearch = slot.doctorName.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesDate = filterDate ? new Date(slot.date).toLocaleDateString() === new Date(filterDate).toLocaleDateString() : true;
            ...
            <Card>
              ...
              <h5 className="text-xl font-bold leading-none text-gray-900 dark:text-white">
               ...
               <Badge color="success" className="text-2xl font-bold">
                 {slots.filter((slot) => slot.status == "Fully Booked").length}
               </Badge>
             </div>
           </Card>
           <Card>
 ...
 <div>
   <p>
     <strong>Total Bookings:</strong> {selectedSlot.totalBookings}
   </p>
 </div>
 <div>
   <p>
     <strong>Booked Count:</strong> {selectedSlot.bookedCount}
   </p>
 </div>
 <div>
   <p>
     <strong>Cancelled Count:</strong> {selectedSlot.cancelledCount || 0}
   </p>
 </div>
 <div>
   <p>
     <strong>Not Booked Count:</strong> {selectedSlot.notBookedCount || 0}
   </p>
 </div>
</div>
</div>
